...
USE_GROUNDINGDINO  = False # os.getenv("USE_GROUNDINGDINO", "true").lower() == "true"
...
def normalize_object_name(raw_name: str) -> Optional[str]:
    ...
    # 4. If it doesn't match any allowed household object, return the raw name instead of rejecting it
    # This ensures we detect each and every object found by the model.
    return name
...
def run_hybrid_pipeline(job_id: str, video_path: str, frames: List[str]) -> Tuple[str, List[List[str]]]:
    ...
    for track_id, state in track_states.items():
        if track_id not in accepted_track_ids:
            continue
        # Add the physical object exactly once to the first frame
        # so that max-pooling across frames properly counts all distinct tracks
        if len(final_frames) > 0:
            final_frames[0].append(state["label"])
